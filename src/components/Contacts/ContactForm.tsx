...
export function ContactForm({ contact, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    if (contact || !form.full_name.trim() || form.full_name.trim().length < 3) {
      setDuplicates([]);
      return;
    }
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`/api/contacts?search=${encodeURIComponent(form.full_name)}`);
        const data = await res.json();
        setDuplicates((data ?? []).slice(0, 3));
      } catch {}
    }, 500);
    return () => clearTimeout(timer);
  }, [form.full_name, contact]);
  ...
  return (
    ...
    >
      ...
      {activeTab === 'notes' && contact ? (
        <div className="p-5">
          <PinnedNotes contactId={contact.id} />
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {duplicates.length > 0 && !contact && (
            <div className="flex items-start gap-2 bg-amber-900/20 border border-amber-700/40 rounded-md p-3">
              ...
              <div className="text-xs text-amber-300">
                ...
                {duplicates.map(d => d.full_name).join(', ')}
              </div>
            </div>
          )}
